...
def parse_args():
    ...
    parser.add_argument(
        '--split-data',
        type=str,
        default='./data/dair-v2x/single-infrastructure-split-data.json',
        required=False,
...
def pcd2bin(pcd_path, bin_path):
    pc1 = pypcd.PointCloud.from_path(pcd_path)
    np_x = (np.array(pc1.pc_data["x"], dtype=np.float32)).astype(np.float32)
    np_y = (np.array(pc1.pc_data["y"], dtype=np.float32)).astype(np.float32)
    np_z = (np.array(pc1.pc_data["z"], dtype=np.float32)).astype(np.float32)
    np_i = (np.array(pc1.pc_data["intensity"], dtype=np.float32)).astype(np.float32) / 255

    points_32 = np.transpose(np.vstack((np_x, np_y, np_z, np_i)))
    points_32.tofile(bin_path)

def get_cam2lidar(root_path, calib_virtuallidar_to_camera_path):
    calib_lidar2cam_path = calib_virtuallidar_to_camera_path
    calib_lidar2cam_path = os.path.join(root_path, calib_lidar2cam_path)
...
def get_lidar2world(root_path, calib_virtuallidar_to_world_path):
    calib_virtuallidar_to_world_path = os.path.join(root_path, calib_virtuallidar_to_world_path)
...
def get_annos(root_path, label_camera_std_path):
    label_path = os.path.join(root_path, label_camera_std_path)
...
def fill_infos(root_path, dataset, max_sweeps=10):
    ...
    for idx in tqdm(range(len(dataset))):        
        ...
        # r_velo2world, t_velo2world = get_lidar2world(root_path, sample["calib_virtuallidar_to_world_path"])
        
        r_velo2world, t_velo2world = np.eye(3), np.zeros((3,))
        r_velo2world_qua = R.from_matrix(r_velo2world).as_quat()
        r_velo2ego_qua = R.from_matrix(np.eye(3)).as_quat()
        can_bus = np.zeros((18,))
        cam_instrinsic_path = os.path.join(root_path, sample["calib_camera_intrinsic_path"]) 
        ...
        info = {
            ...
            'sweeps': [],
            'cams': dict(),
            'scene_token': token,  # temporal related info
            ...
            'timestamp': int(10000000000),

        }

        # pcd2bin(info["lidar_path"], info["lidar_path"] + ".bin")
        info["lidar_path"] = info["lidar_path"] + ".bin"
        ...
        while len(sweeps) < max_sweeps:
            if sample["prev_token"] != "":
                ...
                cam_instrinsic_path = os.path.join(root_path, prev_sample["calib_camera_intrinsic_path"]) 
                calib_intrinsic = get_cam_calib_intrinsic(cam_instrinsic_path)
                sweep = {
        ...
        box_velo = np.array([[0.0, 0.0] for anno in annos]).reshape(-1, 2)
                
        velocity = np.zeros((dims.shape[0], 2))
        valid_flag = np.array([True for anno in annos], dtype=bool).reshape(-1)
            
        names = np.array([map_name_from_general_to_detection[anno["name"].lower()] for anno in annos])
        gt_labels = [anno["name"] for anno in annos]
        for i in range(len(gt_labels)):
            gt_labels[i] = classes.index(
                map_name_from_general_to_detection[gt_labels[i].lower()])
            
        # we need to convert rot to SECOND format.
        gt_boxes = np.concatenate([locs, dims, -rots - np.pi / 2, box_velo], axis=1)
