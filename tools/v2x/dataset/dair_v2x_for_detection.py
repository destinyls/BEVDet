...
class DAIRV2XI(DAIRV2XDataset):
    def __init__(self, path, split_data_path, split="train", sensortype="lidar", extended_range=None):
        ...
        for elem in tqdm(data_infos):
            ...
            gt_label["camera"] = Label(osp.join(path, elem["label_camera_std_path"]), filt)
            gt_label["lidar"] = Label(osp.join(path, elem["label_lidar_std_path"]), filt)
            ...
            elem.update({"prev_token": ""})
            elem.update({"next_token": ""})
            
            self.inf_path2info[token].update({"token": token})
            self.inf_path2info[token].update({"prev_token": ""})
            self.inf_path2info[token].update({"next_token": ""})
            ...
            if sensortype == "camera":
                ...
                # get_annos(path, inf_frame, "camera")
    ...
    def get_split(self, split_path, split, data_infos):
        ...
        for data_info in data_infos:
            ...
            batch_id = data_info["image_path"].split('/')[1].split('.')[0]
